...
export class BreakpointDetectorDirective implements AfterViewInit {
  ...
  constructor (
    ...
    @Inject(BREAKPOINT_DETECTOR_CONFIG) private config: null | BreakpointDetectorConfig
  ) {
  ...
  setBreakPoint(width: number) {
    ...
    } else if (width >= this.config.BREAKPOINT_TABLET_PORTRAIT.value && width < this.config.BREAKPOINT_TABLET_LANDSCAPE.value) {
      this.currentBreakPoint =  this.config.BREAKPOINT_TABLET_PORTRAIT.name;
    } else if (width >= this.config.BREAKPOINT_TABLET_LANDSCAPE.value && width < this.config.BREAKPOINT_MEDIUM_DESKTOP.value) {
      this.currentBreakPoint =  this.config.BREAKPOINT_TABLET_LANDSCAPE.name;
    } else if (width >=  this.config.BREAKPOINT_MEDIUM_DESKTOP.value && width <  this.config.BREAKPOINT_LARGE_DESKTOP.value) {
      this.currentBreakPoint =  this.config.BREAKPOINT_MEDIUM_DESKTOP.name;
    } else if (width >=  this.config.BREAKPOINT_LARGE_DESKTOP.value && width <  this.config.BREAKPOINT_EXTRA_LARGE_DESKTOP.value) {
